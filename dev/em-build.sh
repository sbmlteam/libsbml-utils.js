
export BUILD_DIR="${BUILD_DIR:-em-build}"
export LIBSBML_DEPENDENCY_DIR="${LIBSBML_DEPENDENCY_DIR:-dev/em-dependencies}"
export BUILD_TYPE="${BUILD_TYPE:-Release}"
export ENVIRONMENT="${ENVIRONMENT:-node}"
export INSTALL_PREFIX="${INSTALL_PREFIX:-dist-$ENVIRONMENT}"

emcmake cmake -B $BUILD_DIR \
        -DCMAKE_BUILD_TYPE=$BUILD_TYPE \
        -DLIBSBML_DEPENDENCY_DIR=$LIBSBML_DEPENDENCY_DIR \
        -DLIBSBMLJS_DEPENDENCY_DIR=$LIBSBML_DEPENDENCY_DIR \
        -DCMAKE_INSTALL_PREFIX=$INSTALL_PREFIX \
        -DLIBSBMLJS_ENVIRONMENT=$ENVIRONMENT \
        -DCOPASIJS_INCLUDE_EXAMPLE_FILES=ON

cmake --build $BUILD_DIR --config $BUILD_TYPE

# if the build fails, exit with an error
if [ $? -ne 0 ]; then
    echo "Build failed. Aborting installation."
    exit 1
fi

cmake --install $BUILD_DIR --config $BUILD_TYPE
